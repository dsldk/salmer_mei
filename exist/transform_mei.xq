...
(: Returns a transformed version of the MEI document referred to in the request.    
   ...
   GET example:                                                                     
    transform_mei.xq?doc=Th_1569_LN1426_001r.xml&mdiv=mdiv-01&xsl=comments.xsl
   ...
   The POST request should post a JSON object or XML with contents similar to the following:         
    <post-parameters>
        <doc>Th_1569/Th_1569_LN1426_002r.xml</doc>
        <show>
            <xslt>show.xsl</xslt>
            <parameters>
                <mdiv>mdiv-01</mdiv>
...
let $doc := doc(concat($database,'/',$datadir,'/',$post/post-parameters/doc))/m:mei

let $mei :=   
    if($doc) then   
        let $step1 := local:single_transformation($doc, $show-parameters)    
    ...
    return 
        ...
        concat('Document ',$database,'/',$datadir,'/',$post/post-parameters/doc,' not found')
