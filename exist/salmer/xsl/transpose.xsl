...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <!-- sequence of sharps in key signatures -->
    <xsl:variable name="sharps">fcgdaeb</xsl:variable>
    
    <!-- sequence of flats in key signatures -->
    <xsl:variable name="flats">beadgcf</xsl:variable>
    ...
    <xsl:template name="get_new_key_sig">
        <xsl:param name="keysig"/>
         <xsl:choose>
            <xsl:when test="$dir=1 and $interval &gt; 0">
                <xsl:value-of select="$keys/key[accid=$keysig][1]/following-sibling::key[$interval]/accid"/>
            </xsl:when>
            <xsl:when test="$dir=-1 and $interval &gt; 0">
                <xsl:value-of select="$keys/key[accid=$keysig][2]/preceding-sibling::key[$interval]/accid"/>
            </xsl:when>
            <xsl:otherwise>
    ...
    <xsl:template name="get_latest_key_sig">
        <!-- get current value of @key.sig -->
        <xsl:choose>
            <xsl:when test="ancestor-or-self::m:measure/preceding-sibling::*[@key.sig]">
                <!-- see if there is a preceding scoreDef element re-defining the key signature at <measure> sibling level -->
                <xsl:value-of select="ancestor-or-self::m:measure/preceding-sibling::*[@key.sig][1]/@key.sig"/>
            </xsl:when>
            <xsl:otherwise>
                <!-- otherwise go back to the latest top-level scoreDef to get it -->
                <xsl:value-of select="ancestor::m:section/preceding-sibling::m:scoreDef[//m:staffDef/@key.sig][1]//m:staffDef/@key.sig"/>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:template>
    ...
    <xsl:template match="@key.sig">
        <xsl:attribute name="key.sig">
            <xsl:call-template name="get_new_key_sig">
                <xsl:with-param name="keysig" select="."/>
            </xsl:call-template>
        </xsl:attribute>
    </xsl:template>   
    ...
    <xsl:template match="m:measure">
        <xsl:choose>
            <xsl:when test="$interval != 0">
                <!-- calculate new key signature measurewise (no in-measure key changes allowed) to avoid recalculation for each note -->
                <xsl:variable name="oldKeySig"><xsl:call-template name="get_latest_key_sig"/></xsl:variable>
                <xsl:variable name="oldKey"><xsl:copy-of select="$keys/key[accid=$oldKeySig][1]"/></xsl:variable>
                <xsl:variable name="newKeySig">
                    <xsl:call-template name="get_new_key_sig">
                        <xsl:with-param name="keysig" select="$oldKeySig"/>
                    </xsl:call-template>
                </xsl:variable>
                <xsl:variable name="newKey"><xsl:copy-of select="$keys/key[accid=$newKeySig][1]"/></xsl:variable>
                ...
                <xsl:copy>
                    <xsl:apply-templates mode="transpose" select="@* | node()">
                        <xsl:with-param name="oldKey" select="$oldKey"/>
                        <xsl:with-param name="newKey" select="$newKey"/>
                        <xsl:with-param name="pitchNameDistance" select="$pitchNameDistance"/>
                    </xsl:apply-templates>
                </xsl:copy>
            </xsl:when>
            <xsl:otherwise>
    ...
    <xsl:template match="m:note" mode="transpose">
        <xsl:param name="oldKey"/>
        <xsl:param name="newKey"/>
        <xsl:param name="pitchNameDistance"/>
        <!-- string for calculation of stem tones' position in octave -->
        <xsl:variable name="semitones">cxdxefxgxaxb</xsl:variable>
        ...
        <xsl:copy>
            ...
            <xsl:attribute name="oct"><xsl:value-of select="$oct"/></xsl:attribute>
            <xsl:variable name="pnum" select="@pnum+$dir*$interval"/>
            <xsl:attribute name="pnum"><xsl:value-of select="$pnum"/></xsl:attribute>
            <!-- check whether we need to add @accid.ges -->
            <xsl:variable name="pnumDiff" select="$pnum - (($oct+1)*12 + string-length(substring-before($semitones, $newPitch)))"/>
            <xsl:if test="$pnumDiff != 0">
                <xsl:attribute name="accid.ges">
                    <xsl:choose>
                        <xsl:when test="$pnumDiff = -2">ff</xsl:when>
                        <xsl:when test="$pnumDiff = -1">f</xsl:when>
                        <xsl:when test="$pnumDiff = 1">s</xsl:when>
                        <xsl:when test="$pnumDiff = 2">ss</xsl:when>
                        <xsl:otherwise/>
                    </xsl:choose>  
                </xsl:attribute>
            </xsl:if>           
...
<xsl:attribute name="test"><xsl:value-of select="$newPitch"/><xsl:value-of select="$oct"/></xsl:attribute>            
    ...
    <xsl:template match="@accid" mode="transpose">
        ...
        <xsl:attribute name="{name()}">
            <xsl:choose>
                <xsl:when test="($oldKey/key/accid='0' or contains($oldKey/key/accid,'s')) and contains($newKey/key/accid,'f')">
                    <!-- from a 'sharp' key to a 'flat' key: translate naturals to flats, sharps to naturals -->
                    <xsl:choose>
                ...
                <xsl:when test="($oldKey/key/accid='0' or contains($oldKey/key/accid,'f')) and contains($newKey/key/accid,'s')">
                    <!-- from a 'flat' key to a 'sharp' key: translate naturals to sharps, flats to naturals -->
                    <xsl:choose>
