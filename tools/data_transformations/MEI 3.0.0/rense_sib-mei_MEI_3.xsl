<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <!-- Overwrite existing processing instructions -->
    <xsl:template match="processing-instruction()">
        <!-- Associate MEI XML schema -->
        <xsl:processing-instruction name="xml-model"
            >href="http://www.music-encoding.org/schema/3.0.0/mei-all.rng" type="application/xml"
    ...
    <!-- Ligature brackets (= exported from Sibelius as tuplets with @num=@numbase) -->

    <!-- Use <bracketSpan> (a MEI 4.0.0 feature) -->
    <xsl:template match="m:tuplet[@num=@numbase]">
        <!-- remove note-spanning tuplet element  -->
        <xsl:apply-templates select="*"/>
    </xsl:template>

    <xsl:template match="m:layer[m:tuplet[@num=@numbase]]">
    ...
    <!-- Alternative: keep ligatures as tuplets - NOT IN USE -->
    <xsl:template match="m:tuplet_NOT_IN_USE[@num=@numbase]">
        <tuplet xmlns="http://www.music-encoding.org/ns/mei">
            <xsl:attribute name="label">ligature_bracket</xsl:attribute>
            <xsl:attribute name="bracket.visible">true</xsl:attribute>
            <xsl:attribute name="num.visible">false</xsl:attribute>
            <xsl:apply-templates select="@num | @numbase | @xml:id"/>
            <xsl:apply-templates select="*"/>
        </tuplet>
    </xsl:template>
    <!-- End ligature brackets -->
        ...
        <xsl:choose>
            <xsl:when test="@shape='angular'">
                <dir type="sb" xmlns="http://www.music-encoding.org/ns/mei" tstamp="{$tstamp}"
                    place="above" label="Linjeskift" xml:id="{@xml:id}">|</dir>
            </xsl:when>
            <xsl:otherwise>
                <dir type="pb" xmlns="http://www.music-encoding.org/ns/mei" tstamp="{$tstamp}"
                    place="above" label="Sideskift" xml:id="{@xml:id}">||</dir>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:template>
            ...
            <xsl:choose>
                ...
                <xsl:when test="@dur.ges">
                    <xsl:variable name="slur_dur"
                        select="number(substring-before(@dur.ges,'p')) div 1024"/>
                    <xsl:attribute name="endid">#<xsl:value-of
