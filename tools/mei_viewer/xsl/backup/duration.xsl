...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <!-- change note and rest durations -->
    <xsl:template match="@dur[not(//m:note[@stem.len=0])]">
    ...
    <xsl:template match="@dur.ges[not(//m:note[@stem.len=0])]">
    ...
    <xsl:template match="@tstamp[not(//m:note[@stem.len=0])]">
